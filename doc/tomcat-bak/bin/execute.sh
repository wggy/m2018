...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$CATALINA_HOME/apr/lib
...
if [ $have_tty -eq 1 ]; then
  ...
  echo "Using SERVER_CONF:     $SERVER_CONF"
  echo "Using CATALINA_BASE:   $CATALINA_BASE"
  echo "Using CATALINA_HOME:   $CATALINA_HOME"
...
if [ "$1" = "debug" ] ; then
  if $os400; then
    ...
    if [ "$1" = "-security" ] ; then
      ...
      exec "$_RUNJDB" "$LOGGING_CONFIG" $LOGGING_MANAGER $JAVA_OPTS $CATALINA_OPTS \
        ...
        org.apache.catalina.startup.Bootstrap "$SERVER_CONF" start
    else
      exec "$_RUNJDB" "$LOGGING_CONFIG" $LOGGING_MANAGER $JAVA_OPTS $CATALINA_OPTS \
        -Djava.endorsed.dirs="$JAVA_ENDORSED_DIRS" -classpath "$CLASSPATH" \
        -sourcepath "$CATALINA_HOME"/../../java \
  ...
  eval "\"$_RUNJAVA\"" $LOGGING_MANAGER $JAVA_OPTS \
    ...
    -Dcatalina.home="\"$CATALINA_HOME\"" \
    -Djava.io.tmpdir="\"$CATALINA_TMPDIR\"" \
    org.apache.catalina.startup.Bootstrap "$SERVER_CONF" stop
